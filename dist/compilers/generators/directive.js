...
var metadata_1 = require("@angular/metadata");
...
var DirectiveGenerator = (function (_super) {
    ...
    DirectiveGenerator.prototype.OnEvent = function (key, func) {
        ...
        var directive = function () {
            return {
                ...
                link: function (scope, attr, element, controller) {
                    if (_this.eventMaps.ngOnInit) {
                        _this.eventMaps.ngOnInit();
                    }
                    if (_this.eventMaps.ngOnDestroy) {
                        scope.$on("$destroy", function () { return _this.eventMaps.ngOnDestroy(); });
                    }
                    if (_this.onMaps) {
                        Object.keys(_this.onMaps).forEach(function (name) {
                            scope.$on("$" + name, function () {
                                _this.onMaps[name](scope, attr, element);
                            });
                        });
                    }
                    if (_this.watchMaps) {
                        Object.keys(_this.watchMaps).forEach(function (name) {
                            scope.$watch((_this.config.bindingToController ? (_this.config.alias || "vm") + "." : "") + name, function () {
                                _this.watchMaps[name](scope, attr, element);
                            });
                        });
                    }
                }
